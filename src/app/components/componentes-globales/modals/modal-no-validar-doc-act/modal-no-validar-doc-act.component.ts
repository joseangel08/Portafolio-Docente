...
@Component({
  selector: 'app-modal-no-validar-doc-act',
  templateUrl: './modal-no-validar-doc-act.component.html',
  styleUrls: ['../modal-crear-usuario/modal-crear-usuario.component.css']
})
export class ModalNoValidarDocActComponent implements OnInit {
  ...
  validarDocAct(form:NgForm, documento){
    if(form.valid){
      ...
      swalWithBootstrapButtons.fire({
        ...
        if (result.value) {
          this.servicio.noValidarDocumentoActividad(documento.id_documento, documento.observacion).subscribe(
            data=>{
