...
@Component({
  selector: 'app-modal-cargar-documento-actividad',
  templateUrl: './modal-cargar-documento-actividad.component.html',
  styleUrls: ['./modal-cargar-documento-actividad.component.css']
})
export class ModalCargarDocumentoActividadComponent implements OnInit {
  ...
  guardarArchivo():void{
      ...
      formData.append('id_portafolio_docente', this.servicio.actividadDocentePortafolio.id_act_doc_port);
      ...
      formData.append('path_ubicacion', this.portafolio.path_dir_portafolio+"/"+this.servicio.codigoActividad);
      this.servicio.guardarDocumentoActividad(formData).subscribe((verificador)=>{
        if(verificador!=null){
          if(verificador.verificador==true){
            ...
            this.mensajeNoGuardar(verificador.mensaje);
          }
        }else{
          this.mensajeNoGuardar(verificador.mensaje);
        }
  ...
  obtenerDatosUsuario(){
    ...
    this.servicio.obtenerDatosUsuarioId(user.id).subscribe((usuarios:UsuarioTabla)=>(this.usuarioActivo=usuarios));
  }
